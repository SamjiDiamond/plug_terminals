        ...
        <div class="row">
            <div class="col-sm-12">
                <div class="titleWithDateDiv">
                    <div><h4> <a href="{{route('terminals')}}">< Terminals</a></h4></div>
                    <div>
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="twoItemsPerRow">
                                <div>
                                    <h5 class="grey-color">Assigned To</h5>
                                    <h2>{{$terminal->agent->lastname ?? ''}} {{$terminal->agent->firstname ?? ''}}</h2>
                                    <h6>{{$terminal->agent->phone ?? ''}}</h6>
                                </div>
                                <div>
                                    <h5 class="grey-color">Terminal Serial Number</h5>
                                    <h2>{{$terminal->serial_number}}</h2>
                                </div>
                            </div>
                            @if($terminal->agent_id != null)
                                <a href="{{route('terminal.revoke', $terminal->id)}}" class="materialButtonIcon">
                ...
                <div class="materialCard materialTableContainer mt-5">
                    <div class="materialCardTitle">
                        <div class="materialCardInline flex-between">
                            <div>
                                <h4 id="lists">Terminal Transactions</h4>
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                @foreach($transactions as $term)
                                    <tr>
                                        <td>{{$term->reference}}</td>
                                        <td>N{{number_format($term->amount)}}</td>
                                        <td>{{$term->transactionType}}</td>
                                        <td>N{{number_format($term->balance)}}</td>
                                        <td>{{$term->timeCreated}}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
