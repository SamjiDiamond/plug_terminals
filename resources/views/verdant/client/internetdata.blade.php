...
@section('contents')
    <div class="container-fluid dashboardSummary">
        ...
        @include('verdant.admin.partials.client_vas_nav', ['active' => "d"])
            ...
            <div class="materialCard materialTableContainer mt-5">
                <div class="materialCardTitle">
                    <div class="materialCardInline flex-between">
                        <div>
                            <h4 id="lists">Internet-Dataplan Table</h4>
                ...
                <div class="table-responsive">
                    <table class="table table-borderless">
                        ...
                        @foreach($all as $cli)
                            <tr>
                                <td>{{$cli->parentData->type}}</td>
                                <td>{{$cli->parentData->name}}</td>
                                <td>{{$cli->parentData->price}}</td>
                                <td>{{$cli->price}}</td>
                                <td>{{$cli->c_cent}}</td>
                                <td>
                                    @if($cli->status=='1')
                                ...
                                <td><a href="{{route('editData',$cli->id)}}">
                                    ...
                                    <label class="toggleSwitch nolabel">
                                        <input type="checkbox" name="status" value="0" id="myCheckBox"
                                               {{$cli->status =="1"?'checked':''}} onclick="window.location='{{route('dataModify', $cli->id)}}'"/>
                                        <span>
                                                <span>off</span>
                                                <span>on</span>
                                             </span>
