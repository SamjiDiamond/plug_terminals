...
@section('contents')
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless dataTable">
                            ...
                            <tr>
                                ...
                                <th scope="col">Total Service / Cap Fee</th>
                                <th scope="col">Varst Service / Cap Fee</th>
                                <th scope="col">My Service / Cap Fee</th>
                            ...
                            @foreach($list as $serv)
                                <tr>
                                    <td>{{$serv->id}}</td>
                                    <td>{{$serv->parentData->name}}</td>
                                    <td>{{$serv->parentData->fee + $serv->fee}} @if($serv->fee_type == 0) Flat @else % - NGN{{$serv->parentData->capped_fee + $serv->capped_fee}} @endif</td>
                                    <td>{{$serv->parentData->fee}} @if($serv->parentData->fee_type == 0) Flat @else % - NGN{{$serv->parentData->capped_fee}} @endif</td>
                                    ...
                                    <div class="materialModalContainer viewTransactionDetailsDiv" data-trigger-content="editServices{{$serv->id}}">
                                        <div class="materialModalDiv">
                                            ...
                                            <form action="{{route('updateFee')}}" method="POST">
                                                @csrf

                                                <div class="materialModalHeader" style="margin-bottom: 1rem;">
                                                    <div>
                                                        <h4>{{$serv->parentData->name}}</h4>
                                                        <h6>{{$serv->parentData->description}}</h6>
                                                    </div>
    ...
    <!-- Modal Container -->
    <div class="materialModalContainer viewTransactionDetailsDiv" data-trigger-content="editServices">
        <div class="materialModalDiv">
            <div class="materialModalHeader" style="margin-bottom: 1rem;">
                <div>
                    <h4>Varst Service / Cap Fee</h4>
                </div>
                <div class="closeModal">
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.5 1.5L22.8627 22.5627" stroke-linecap="round" stroke-linejoin="round"/><path d="M22.8625 1.5L1.49986 22.5627" stroke-linecap="round" stroke-linejoin="round"/></svg>
                </div>
            ...
            <div class="materialCardForm">
                <input type="text" class="firstName" placeholder="Varst Service / Cap Fee" id="">
                <input type="submit" class="btn-primary" style="margin-top: 0.25rem;">
            </div>
        </div>
    </div>
