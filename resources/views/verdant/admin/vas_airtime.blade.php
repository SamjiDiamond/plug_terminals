...
@section('contents')
    <div class="container-fluid dashboardSummary">
        ...
        @include('verdant.admin.partials.vas_nav', ['active' => "a"])
                ...
                <div id="transactionsTable" class="materialCard materialTableContainer mt-5">
                    ...
                    <div class="table-responsive">
                        <table class="table table-borderless">
                            ...
                            <tbody>
                                @foreach($airtimes as $trans)
                                    <tr>
                                        <td>{{$i++}}</td>
                                    <td>{{$trans->provider}}</td>
                                    <td>&#8358; {{$trans->minAmount}}</td>
                                    <td>&#8358; {{$trans->maxAmount}}</td>
                                    <td>{{$trans->c_cent}}</td>
                                    <td>
                                        @if($trans->status == 1)
                                    ...
                                    <td>{{\Carbon\Carbon::parse($trans->created_at)->format('Y-m-d')}}</td>
                                        <td>
                                            <a href="{{route('admin.editairtime',$trans->id)}}">
                                            ...
                                            <label class="toggleSwitch nolabel">
                                                <input type="checkbox" name="status" value="0" id="myCheckBox"
                                                       {{$trans->status =="1"?'checked':''}} onclick="window.location='{{route('admin.airtimeswitch', $trans->id)}}'"/>
                                                <span>
                                                <span>off</span>
                                                <span>on</span>
                                             </span>
