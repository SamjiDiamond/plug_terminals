...
@section('contents')
    <div class="container-fluid dashboardSummary">
        ...
        @include('verdant.admin.partials.vas_nav', ['active' => "c"])
                            ...
                            <tbody>
                                @foreach($data as $trans)
                                    ...
                                    <td>{{\Carbon\Carbon::parse($trans->created_at)->format('Y-m-d')}}</td>
                                        <td>
                                            <a href="{{route('admin.edittv',$trans->id)}}">
                                            ...
                                            <label class="toggleSwitch nolabel">
                                                <input type="checkbox" name="status" value="0" id="myCheckBox"
                                                       {{$trans->status =="1"?'checked':''}} onclick="window.location='{{route('admin.tvswitch', $trans->id)}}'"/>
                                                <span>
                                                <span>off</span>
                                                <span>on</span>
                                             </span>
